...
<!-- CSS의 정의가 다소 딱딱하게 느껴질 수 있는데, CSS 등장 이전/이후의 스타일링 방법의 차이를 보면 조금 더 이해에 도움이 될 것 같습니다.  -->
...
‘페이지 수가 늘어남에 따라 CSS 가 점점 복잡해지고 관리하기 어렵다’ 는 점입니다.
...
다음과 같은 기본 버튼과 취소 버튼을 스타일해야한다고 해보겠습니다.
...
위와 같이 일반적으로 구분할 수 있지만 OOCSS 에서는 명확하게 결정되어 있는 것은 아닙니다.
...
컨테이너는 ‘영역’, 콘텐츠는 '모듈'을 의미합니다. 바로 앞의 예시에서는 id 속성에 ‘ main ’이 지정된 main 요소가 컨테이너, 버튼이 모듈에 해당합니다.
...
다음은 스테이트 규칙입니다. 스테이트 규칙은 기존 스타일을 덮어쓰거나 확장하기 위해 사용합니다.
 ...
 기존 스타일을 모두 덮어써서 상태 스타일을 반영하는 것을 기대하기 때문에 필요한 경우에는 `!important` 사용도 권장하고 있습니다.
